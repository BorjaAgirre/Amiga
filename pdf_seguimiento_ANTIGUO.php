...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		$fill = 0;
		foreach($data as $row) {
			$this->Cell($w[0], 6, $row[0], 'LR', 0, 'L', $fill, '', 0);
			$this->Cell($w[1], 6, $row[1], 'LR', 0, 'L', $fill, '', 0);
...
foreach ($comentarios as $com) {
    ...
    if ($com['comentario'] != '') { $data[] = array(0 => $com['fecha'], 1 => $com['comentario']); }
}
...
$pdf->Output('pdf_seguimiento.pdf', 'I');
