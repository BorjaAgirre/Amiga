...
if ((isset($_POST['action']) ? $_POST['action'] : (isset($_GET['action']) ? $_GET['action'] : '')) == 'simple') {
    ...
    if ($handle->uploaded) {
        ...
        echo '  Error (linea 128): ' . $handle->error . '';
	echo ' path: ';
        echo '</fieldset>';
    }
    ...
    if ($handle->uploaded) {
        ...
        echo '<fieldset>';
        echo '  <legend>file not uploaded on the server</legend>';
        echo '  Error (linea 210): ' . $handle->error . '';
	echo ' <br>Path: '.$handle->file_dst_path.'';
	echo ' <br>Dst name: '.$handle->file_dst_name.'';
	echo ' <br>Src name: '.$handle->file_src_name.'';
        echo '</fieldset>';
/*	echo '<pre>';
	print_r($_FILES); 
	echo '</pre>';*/
    ...
    foreach ($files as $file) {
        ...
        if ($handle->uploaded) {
            ...
            echo '<fieldset>';
            echo '  <legend>file not uploaded on the server</legend>';
            echo '  Error (linea 269): ' . $handle->error . '';
            echo '</fieldset>';
        }
    }
